...
#include "include/syncdly.h"   
                                
#include "include/FoLogic.h"
...
void DSLogic_Init(void)
{
  ...
  SYNCDELAY;           
  EP6CFG = 0xE0;     // EP6IN, bulk, size 512, single buffered
  SYNCDELAY;
  ...
  // EP6 Configuration
  EP6FIFOCFG = 0x08; 	// auto in mode, word ops
  SYNCDELAY;
  EP6AUTOINLENH = 0x02;	// Auto-commit 512-byte packets
  SYNCDELAY;
  EP6AUTOINLENL = 0x00;
  SYNCDELAY;
  EP6GPIFFLGSEL = 0x02;	// For EP6IN, GPIF uses FF (Full Flag)
  SYNCDELAY;
...
void DSLogic_Poll(void)
{
 ...
 //     cfg_enable= FALSE;                 	//end of configuration
      ...
      // Simplified: Set LED indicators for configuration completion
 //     IOA |= 0x01;
 //     IOA &= 0xf5;
 //     EZUSB_Delay1ms();
 //     IOA |= 0x08;
  ...
  switch(command)
  {
    ...
    case CMD_WR_REG:
    {
      if ((EP0CS & bmEPBUSY) != 0)
        break;
      if (EP0BCL == sizeof(struct cmd_wr_reg))
	  {
        // Parse register write command data (16-bit value)
        const struct cmd_wr_reg *wr_cmd = (const struct cmd_wr_reg *)EP0BUF;
        wr_reg_value = wr_cmd->reg_value;
        
        // Set flag to enable register write
        wr_reg_enable = TRUE;
        
        // Set LED indicator for register write
     //   IOA |= 0x02;  // Set LED for register write operation
      }
	  command = 0;
	  break;
    }
  ...
  // CMD_WR_REG GPIF transmission
  if (wr_reg_enable && (GPIFTRIG & 0x80)) {  // if GPIF interface IDLE
    if (!(EP24FIFOFLGS & 0x04)) {
  //    SYNCDELAY;
  //    GPIFTCB2 = 0x00;                       // Padding byte
      SYNCDELAY;
      GPIFTCB1 = (wr_reg_value >> 8) & 0xFF; // High byte: register address
      SYNCDELAY;
      GPIFTCB0 = wr_reg_value & 0xFF;        // Low byte: data value
      SYNCDELAY;
      ...
      // Clear LED indicator
    //  IOA &= 0xFD;  // Clear register write LED
    }
  }
  poll_intf();  
}
...
BOOL DR_VendorCmnd(void)
{
  switch (SETUPDAT[1])
  {
	...
	{
	  EP0BUF[0] = FOLOGICFW_VER_MAJOR;
	  EP0BUF[1] = FOLOGICFW_VER_MINOR;
	  EP0BCH = 0;
	  EP0BCL = 2;
	  break;
	}
